import { fetchUpcomingSchedule } from "@/lib/teamup-schedule";
...
export async function GET(request: Request) {
  ...
  try {
    const schedule = await fetchUpcomingSchedule(days);
    return Response.json({ ok: true, durationMs: Date.now() - startedAt, ...schedule });
  } catch (err) {
